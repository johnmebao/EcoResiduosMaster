    ...
    <div class="container-fluid">
        <x-adminlte-card theme="success" theme-mode="outline" icon="fas fa-plus-circle" title="Formulario de Nueva Recolección">
            <form action="{{ route('collections.store') }}" method="POST" id="collection-form">
                ...
                <div class="row">
                    {{-- Usuario --}}
                    <x-adminlte-input name="user_id" label="Usuario" value="{{ Auth::user()->name }}" label-class="text-primary" disabled>
                        ...
                        <x-adminlte-select2 name="localidad_id" label="Localidad" label-class="text-info"
                            ...
                            @foreach ($localidades as $localidad)
                                <option value="{{ $localidad->id }}"
                                    {{ old('localidad_id') == $localidad->id ? 'selected' : '' }}>
                                    {{ $localidad->nombre }}
                                </option>
                            @endforeach
                        </x-adminlte-select2>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        @endphp
                        <x-adminlte-input-date name="fecha_programada" label="Fecha Programada" label-class="text-warning"
                            :config="$config" value="{{ old('fecha_programada', date('Y-m-d')) }}"
                ...
                <div id="peligrosos-section" style="display: none;">
                    <hr>
                    <h5 class="text-danger"><i class="fas fa-exclamation-triangle"></i> Información de Residuos Peligrosos
                    </h5>

                    <div class="row">
                        {{-- Estado de Solicitud --}}
                        <div class="col-md-6">
                            <x-adminlte-select name="estado_solicitud" label="Estado de Solicitud"
                                ...
                                @foreach (App\Models\Collection::getEstadosSolicitud() as $key => $estado)
                                    <option value="{{ $key }}"
                                        {{ old('estado_solicitud') == $key ? 'selected' : '' }}>
                                        {{ $estado }}
                                    </option>
                                @endforeach
                            </x-adminlte-select>
                            ...
                            <x-adminlte-select2 name="aprobado_por" label="Aprobado Por" label-class="text-success"
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}"
                                        {{ old('aprobado_por') == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </x-adminlte-select2>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <x-adminlte-textarea name="motivo_rechazo" label="Motivo de Rechazo" rows="3"
                                label-class="text-danger" placeholder="Ingrese el motivo del rechazo...">
                                <x-slot name="prependSlot">
                                    <div class="input-group-text bg-gradient-danger">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <x-adminlte-textarea name="notas" label="Notas" rows="3" label-class="text-info"
                ...
                <div class="row">
                    <div class="col-md-12">
                        <x-adminlte-button type="submit" label="Guardar Recolección" theme="success"
                            icon="fas fa-save" />
